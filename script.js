...
function bibTeXToXML(data) {
    ...
    const xmlTemplates = {
        ...
        JournalArticle: () => `<b:Source><b:Tag>${data.key || 'Unknown'}</b:Tag><b:SourceType>JournalArticle</b:SourceType><b:Guid>{${generateGUID().toUpperCase()}}</b:Guid><b:Title>${data.title || ''}</b:Title><b:Year>${data.year || ''}</b:Year><b:Pages>${(data.pages || '').replace('–', '-')}</b:Pages><b:City>${data.address || ''}</b:City><b:Publisher>${data.publisher || ''}</b:Publisher><b:JournalName>${data.journal || ''}</b:JournalName><b:Volume>${data.volume || ''}</b:Volume><b:Issue>${data.number || ''}</b:Issue><b:ShortTitle>${data.shorttitle || ''}</b:ShortTitle><b:StandardNumber>${data.issn || ''}</b:StandardNumber><b:Comments>${data.note || ''}</b:Comments><b:Medium>${data.medium || ''}</b:Medium><b:YearAccessed>${data.yearaccessed || ''}</b:YearAccessed><b:MonthAccessed>${data.monthaccessed || ''}</b:MonthAccessed><b:DayAccessed>${data.dayaccessed || ''}</b:DayAccessed><b:URL>${data.url || ''}</b:URL><b:DOI>${data.doi || ''}</b:DOI><b:Author><b:Author><b:NameList><b:Person><b:Last>${lastName}</b:Last></b:Person></b:NameList></b:Author><b:Editor><b:NameList><b:Person><b:Last>${editorName}</b:Last></b:Person></b:NameList></b:Editor></b:Author></b:Source>`,
